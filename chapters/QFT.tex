...
\begin{enumerate}
	...
	where $D_F(x-y)$ is used instead of $G$ to denote that it is the propagator for the real scalar field. The propagator is both defined in real space and momentum space. The real space representation is found by insertion of the field into the definition,  i.e.
	 \begin{equation}
		\begin{split}
			D_F(x-y)&=\braket{0|T\{\phi(x)\phi(y)\}|0}\\
			&=\bra{0}\int \frac{d^3 p}{(2\pi)^3}\frac{1}{\sqrt{2E_{\vec{p}}}}\bigg(a_{\vec{p}}\,e^{-ip\cdot x}+a_{\vec{p}}^\dagger\,e^{ip\cdot x}\bigg)\\
				&\qquad\int \frac{d^3 q}{(2\pi)^3}\frac{1}{\sqrt{2E_{\vec{q}}}}\bigg(a_{\vec{q}}\,e^{-iq\cdot y}+a_{\vec{q}}^\dagger\,e^{iq\cdot y}\bigg)\ket{0}.\\
		\end{split}
	\end{equation} 
...
\begin{equation}
	\begin{split}
		...
		\psi(x)&=v(\vec{p}\,)e^{ip\cdot x}=\begin{bmatrix}
			v_L(\vec{p}\,)\\
			v_R(\vec{p}\,)\\
		\end{bmatrix}e^{ip\cdot x}.
	\end{split}
...
\begin{enumerate}
	...
	\begin{equation}
		\begin{split}
			[A^\mu(\vec{x},t), \Pi^\nu(\vec{y},t)] &= ig^{\mu\nu}\delta^{(3)}(\vec{x}-\vec{y})\\ [A^\mu(\vec{x},t),A^\nu(\vec{y},t)]& = [\Pi^\mu(\vec{x},t),\Pi^\nu(\vec{y},t)]=0.
		\end{split}
	\end{equation} 
	...
	\begin{equation}
		\begin{split}
			\braket{\vec{p},\lambda|\vec{p},\lambda} &= 2E_{\vec{p}}\braket{0|a_{\vec{p},\lambda}a_{\vec{p},\lambda}^\dagger|0}\\
			&=2E_{\vec{p}}\braket{0|[a_{\vec{p},\lambda},a_{\vec{p},\lambda}^\dagger]|0}\\
			&=-2(2\pi)^3\delta^{(3)}(0)\\
			&<0.
		\end{split}
	\end{equation}  
...
\begin{equation}
	\begin{split}
		I&=i\int d^4x\bigg(-\frac{1}{2}\bigg(\phi'+i\int d^4y D_F(x-y)J(y)\bigg)(\square-m^2)\\
		&\quad\cdot\bigg(\phi'+i\int d^4z D_F(x-z)J(z)\bigg)+J\bigg(\phi'+i\int d^4y D_F(x-y)J(y))\bigg)\\
		&=i\int d^4x\bigg(-\frac{1}{2}\phi'(\square-m^2)\phi'+J\phi'+i\int d^4yJ(x) D_F(x-y)J(y)\\
		&\quad-\frac{1}{2}i\int d^4y D_F(x-y)J(y)(\square-m^2)\phi'-\frac{1}{2}\phi'(\square-m^2)i\int d^4z D_F(x-z)J(z)\\
		&\quad-\frac{1}{2}i\int d^4y D_F(x-y)J(y)(\square-m^2)i\int d^4z D_F(x-z)J(z)\bigg).\\
	\end{split}
\end{equation} 
...
Now that the generating functional has been reformulated in a simpler form containing the propagator the propagator of the free field theory can be determined
 \begin{equation}
	\begin{split}
		...
		&=-\frac{\delta}{\delta J(x_1)}\bigg((-\frac{1}{2}\int d^4yD_F(x-y)J(y)\\
		&\qquad-\frac{1}{2}\int d^4x J(x)D_F(x-y))Z[J]\bigg)\bigg|_{J=0}\\
		&=D(x_2-x_1),
	\end{split}
\end{equation} 
...
\begin{equation}
		\begin{split}
			Z[\eta,\bar{\eta}]&=\frac{1}{z[0,0]}\int D\bar{\psi}\int D\psi e^{i\int d^4x(\bar{\psi}(i\slashed \partial -m)\psi+\bar{\eta}\psi+\bar{\psi}\eta)}\\
			&\equiv\frac{1}{z[0,0]}\int D\bar{\psi}\int D\psi e^{I}.
		\end{split}
\end{equation} 	
...
\begin{equation}
	\begin{split}
		\psi(x)&=\psi'(x)+i\int d^4yS_F(x-y)\eta(y),\\
		\bar{\psi}(x)&=\bar{\psi}'(x)-i\int d^4z\bar{\eta}(z)S_F(x-z).
	\end{split}
\end{equation} 
Hereby
\begin{equation}
	\begin{split}
		...
		&=i\int d^4x\bigg(\bar{\psi}'(i\slashed \partial-m)\psi'+\bar{\psi}'(i\slashed \partial)i\int d^4yS_F(x-y)\eta(y)\\
		& \quad-i\int d^4z\bar{\eta}(z)S_F(x-z)(i\slashed \partial -m)\psi'\\
		&\quad-i\int d^4z\bar{\eta}(z)S_F(x-z)(i\slashed \partial-m)i\int d^4yS_F(x-y)\eta(y)+\bar{\eta}\psi'\\
		&\quad + i\bar{\eta}\int d^4yS_F(x-y)\eta(y)+\bar{\psi}'\eta-i\int d^4z\bar{\eta}(z)S_F(x-z)\eta(x)\bigg).\\
	\end{split}
\end{equation} 
Now use that
\begin{equation}
...
Hereby
 \begin{equation}
	\begin{split}
		I=i \int d^4x \bigg(&\frac{1}{2}A'_\nu f^{\mu\nu}A'_\mu+\frac{1}{2}A'_\nu f^{\mu\nu}i\int d^4y G_{\mu\alpha}(x-y)J^\alpha(y)\\
		&+\frac{1}{2}i\int d^4z G_{\nu\beta}(x-z)J^\beta(z)f^{\mu\nu}A'_\mu\\
		&+\frac{1}{2}i\int d^4z G_{\nu\beta}(x-z)J^\beta(z)f^{\nu\mu}i\int d^4y G_{\mu\alpha}(x-y)J^\alpha(y)\\
		&+J^\mu A'_\mu+J^\mu i\int d^4y G_{\mu\alpha}(x-y)J^\alpha(y)\bigg).\\
	\end{split}
\end{equation} 
Now use that
\begin{equation}
...
\begin{equation}
	\begin{split}
		U(x+n\varepsilon,x)\psi(x)\Rightarrow &e^{i\alpha(x+n\varepsilon)}U(x+n\varepsilon,x)\psi(x)e^{-i\alpha(x)}e^{i\alpha(x)}\psi(x)\\
		&=e^{i\alpha(x+n\varepsilon)}U(x+n\varepsilon,x)\psi(x),
	\end{split}
\end{equation} 
...
\begin{equation}
	\begin{split}
		n^\mu D_\mu \psi&=\lim\limits_{\varepsilon\Rightarrow 0}\bigg(\frac{1}{\varepsilon}(\psi(x+n\varepsilon)-\psi(x)+ie\varepsilon n^\mu A_\mu(x)\psi(x))\bigg)\\
		&=n^\mu \partial_\mu\psi+ien^\mu A_\mu\psi\Rightarrow D_\mu\\
		&=\partial_\mu+ieA_\mu.
	\end{split}
\end{equation}  
...
\begin{equation}
	\begin{split}
		1-ie\varepsilon n^\mu A_\mu\Rightarrow &(1-ie\varepsilon n^\mu A_\mu)(1-i(\alpha(x+n\varepsilon)-\alpha(x)))\\
		&=1-ie\varepsilon n^\mu (A_\mu-\frac{1}{e}\partial_\mu\alpha(x)),
	\end{split}
\end{equation}  
...
\begin{equation}
	\begin{split}
		...
		&=\cancelto{0}{[\partial_\mu,\partial_\nu]}\psi+ie([\partial_\mu,A_\nu]-[\partial_\nu,A_\mu])\psi+e^2\cancelto{0}{[A_\mu,A_\nu]}\psi\\
		&=ie(\partial_\mu A_\nu-\partial_\nu A_\mu)\psi.\\
	\end{split}
\end{equation}  
...
\begin{equation}
	\begin{split}
		...
		&=-ig([\partial_\mu,A_\nu^b]t^b+A_\nu^b\cancelto{0}{[\partial_\mu,t^b]})\psi-ig(A_\mu^a\cancelto{0}{[t^a,\partial_\nu]}+[A_\mu^a,\partial_\nu]t^a)\psi\\
		&-g^2(A_\mu^a\cancelto{0}{[t^a,A_\nu^b]}+A_\mu^aA_\nu^b\underbrace{[t^a,t^b]}_{=if^{abc}t^c}+\underbrace{[A_\mu^a,A_\nu^b]}_{=0}t^bt^a+A_\nu^b\cancelto{0}{[A_\mu^a,t^b]}t^a)\psi\\
...
\begin{equation}
	\begin{split}
		...
		&+\underbrace{gA_\mu^a\bar{\psi}\gamma^\mu t^a\psi}_{\text{From }D_\mu}\\
		&\cdot\underbrace{-gf^{abc}(\partial_\mu A_\nu^a)(A^b)^\mu(A^c)^\nu-\frac{g^2}{2}(f^{eab}A_\mu^aA_\nu^b)(f^{ecd}(A^c)^\mu(A^d)^\nu)}_{\text{From F-term}}
	\end{split}
\end{equation} 

\begin{equation}
	gA_\mu^a\bar{\psi}\gamma^\mu t^a \psi \Rightarrow
	% [scale=0.7, transform shape] mimics the "small" option without breaking anything
	\begin{tikzpicture}[baseline=(b.base), scale=0.7, transform shape]
		\begin{feynman}
			% Explicit coordinates for the perfect Y shape
			\vertex (d) at (0, -1.5) {\(a,\mu\)};     % Bottom center
			\vertex (b) at (0, 0);                    % Central vertex
			\vertex (a) at (-1.5, 1.2) {\(\psi\)};    % Top Left (slanting up)
			\vertex (c) at (1.5, 1.2) {\(\bar{\psi}\)};% Top Right (slanting up)
...
\begin{equation}
	...
	\begin{tikzpicture}[baseline=(b.base), scale=0.7, transform shape]
		\begin{feynman}
			% Upright Y-shape coordinates
			\vertex (a) at (0, -1.5) {\(a, \mu\)};       % Bottom Center
			\vertex (b) at (0, 0);                      % Central interaction point
			\vertex (d) at (-1.5, 1.2) {\(c, \alpha\)}; % Top Left
			\vertex (c) at (1.5, 1.2) {\(b, \nu\)};     % Top Right
			
			\diagram* {
				% Bottom vertical line: momentum k pointing up toward the center
				(a) -- [boson, momentum=\(k\)] (b),
				
				% Top Left branch: drawn outer-to-inner (downward arrow). 
				% Changed to momentum' to push it to the left/outside.
				(d) -- [boson, momentum'=\(q\)] (b),
				
				% Top Right branch: drawn outer-to-inner (downward arrow). 
				% Changed to momentum to push it to the right/outside.
				(c) -- [boson, momentum=\(p\)] (b),
			};
		\end{feynman}
	\end{tikzpicture}
...
\begin{equation}
		...
		\begin{tikzpicture}[baseline=(b.base), scale=0.7, transform shape, ampersand replacement=\&]
			\begin{feynman}
				\vertex (b) at (0, 0);
				\vertex (d) at (-1.5,  1.2) {\(a, \mu\)};   % Top Left
				\vertex (c) at ( 1.5,  1.2) {\(c, \alpha\)}; % Top Right
				\vertex (e) at ( 1.5, -1.2) {\(b, \nu\)};   % Bottom Right
				\vertex (a) at (-1.5, -1.2) {\(d, \beta\)};  % Bottom Left
				
				\diagram* {
					% Proper X: connect opposite corners through the center
					(d) -- [boson] (b) -- [boson] (e),
					(a) -- [boson] (b) -- [boson] (c),
				};
			\end{feynman}
...
\begin{equation}
	\begin{split}
		\int DA e^{iS} &=det\bigg(\frac{\delta G(A)}{\delta \alpha}\bigg) \underbrace{\int D\alpha}_{\text{What diverges}} \int D\omega e^{-i\int d^4x \frac{\omega^2}{2\xi}}\int DA e^{iS}\\
		&\propto \int DAe^{iS-i\int d^4x \frac{(\partial_\mu A^\mu)^2}{2\xi}}.
	\end{split}
\end{equation} 
...
\begin{equation}
	\begin{split}
		\mathcal{L}_{fermion}\supset \sum_{generations}\bigg(&\bar{E}_L(i\slashed D)E_L+\bar{e}_R(i\slashed D)e_R+\bar{Q}_L(i \slashed D)Q_L\\
		&+\bar{u}_R(i \slashed D)u_R+\bar{d}_R(i \slashed D)d_R\bigg),
	\end{split}
\end{equation} 
...
\begin{equation}
	\mathcal{L}_{fermion}\supset \sum_{generations}\bigg((-1)\lambda_e\bar{E}_L\phi e_R-\lambda_d\bar{Q}_L\phi d_R-\lambda_u\varepsilon^{ab}\bar{Q}_L^a\phi^{b\dagger}u_R+\text{h.c.}\bigg),
\end{equation} 
...
\begin{equation}
	\begin{split}
		...
		&\quad+\sum_{generations}\bigg(\bar{E}_L(i\slashed D)E_L+\bar{e}_R(i\slashed D)e_R+\bar{Q}_L(i \slashed D)Q_L\\
		&\qquad\qquad+\bar{u}_R(i \slashed D)u_R+\bar{d}_R(i \slashed D)d_R\bigg)\\
		&\quad+\sum_{generations}\bigg((-1)\lambda_e\bar{E}_L\phi e_R-\lambda_d\bar{Q}_L\phi d_R-\lambda_u\varepsilon^{ab}\bar{Q}_L^a\phi^{b\dagger}u_R+\text{h.c.}\bigg).\\
	\end{split}
\end{equation} 
...
\begin{equation}
	\begin{split}
		\int d\Phi^{(2)}&=\int\frac{d^3p_1'}{(2\pi)^3}\frac{1}{2E_1 '}(2\pi)^4\delta(E_1+E_2-E_1'-E'_2)\\
		&\qquad\cdot\int\frac{d^3p'_2}{(2\pi)^3}\frac{1}{2E'_2}\delta^{(3)}(\vec{p}_1+\vec{p}_2-\vec{p'}_1-\vec{p'}_2)\\
...
\end{equation} 
By using equation \eqref{eq:phase-space-integral} and \eqref{obs} the differential cross section can, in the CM rest frame, be written as
\begin{equation}
...
where the positive sign comes from the fact that $a_{\vec{p}_1}^{in \dagger}-a_{\vec{p_1}}^{out\dagger}\propto\lim_{t\Rightarrow -\infty} -\lim_{t\Rightarrow \infty}$. Applying the time derivatives and writing it out
 \begin{equation}
	\begin{split}
		...
		&=iZ^{-\frac{1}{2}}\int d^4x (-iE_{\vec{p}_1}e^{-ip_1\cdot x}\phi+e^{-ip_1\cdot x}\partial_0^2\phi\\
		&\qquad+E_{\vec{p}_1}^2e^{-ip_1\cdot x}\phi+iE_{\vec{p}_1}e^{-ip_1\cdot x}\phi) \\
...
\begin{equation}
	\begin{split}
		\braket{f|iT|i}&=\bigg(i\frac{1}{\sqrt{Z}}\bigg)^{n+m}\int\prod_{i=1}^m d^4x_i\int\prod_{j=1}^n d^4y_ie^{i(p'_j\cdot y_j-p_i\cdot x_i)}(\square _{x_i}+m_{\vec{p}_i}^2)\\
		&\quad\cdot(\square _{y_j}+m_{\vec{p'}_j}^2)\braket{\Omega|T\{\phi(x_1)\dots\phi(x_m)\phi(y_1)\dots \phi(y_n)\}|\Omega}.\\
	\end{split}
	\label{am1}
\end{equation} 
...
\begin{equation}
	\braket{\Omega|T\{\phi(x_1)\dots \phi(x_N)\}|\Omega}=\lim\limits_{T\Rightarrow \infty (1-i\varepsilon)}(B)
\end{equation} 
where to fit on the page
\begin{equation}
	B\equiv \frac{\braket{0|U^\dagger(t_0,T)[U(t_0,t)\phi_I(x_1)U(t,t_0)]\dots U(t_0,-T)|0}}{e^{-iE_0(2T)}|\braket{\Omega|0}|^2}.
\end{equation}
To get rid of the denominator, I divide by
\begin{equation}
	1=\braket{\Omega|\Omega}=\frac{\braket{0|U(T,t_0)U(t_0,-T)|0}}{e^{-iE_0(2T)}|\braket{\Omega|0}|^2}.
\end{equation} 
Next collect the fields in the nominator under a time-ordering. By doing so the time-evolution operators can be collected to the exponential. Hereby the n-point function can be written with
\begin{equation}
	B = \frac{\braket{0|T\{\phi_I(x_1)\dots\phi_I(x_N)e^{-i\int d^4x \mathcal{H}_I}\}|0}}{\braket{0|T\{e^{-i\int d^4x \mathcal{H}_I}\}|0}}
	\label{pert}
\end{equation} 
...
\begin{equation}
	\begin{split}
		...
		&=\frac{\big(i\frac{1}{\sqrt{Z}}\big)^{n+m}}{(2\pi)^4\delta^{(4)}(\Sigma p'-\Sigma p)}\int\prod_{i=1}^m d^4x_i\int\prod_{j=1}^n d^4y_ie^{i(p'_j\cdot y_j-p_i\cdot x_i)}(\square _{x_i}+m_{\vec{p}_i}^2)\\
		&\qquad \cdot(\square _{y_j}+m_{\vec{p'}_j}^2)\braket{\Omega|T\{\phi(x_1)\dots\phi(x_m)\phi(y_1)\dots \phi(y_n)\}|\Omega}\\
		&=\frac{\big(i\frac{1}{\sqrt{Z}}\big)^{n+m}}{(2\pi)^4\delta^{(4)}(\Sigma p'-\Sigma p)}\int\prod_{i=1}^m d^4x_i\int\prod_{j=1}^n d^4y_ie^{i(p'_j\cdot y_j-p_i\cdot x_i)}(\square _{x_i}+m_{\vec{p}_i}^2)\\
		&\qquad\cdot (\square _{y_j}+m_{\vec{p'}_j}^2)\lim\limits_{T\Rightarrow \infty (1-i\varepsilon)}\bigg(\frac{\braket{0|T\{\phi_I(x_1)\dots\phi_I(y_n)e^{-i\int d^4x \mathcal{H}_I}\}|0}}{\braket{0|T\{e^{-i\int d^4x \mathcal{H}_I}\}|0}}\bigg). 
	\end{split}
	\label{qw}
\end{equation} 
...
\begin{example}
	...
	\begin{enumerate}
		...
		\begin{equation}
			\begin{split}
				&\bigg(\prod_{i=1}^{2}\frac{i\sqrt{Z}}{p'_i-m^2}\bigg)\bigg(\prod_{j=1}^{2}\frac{i\sqrt{Z}}{p_j-m^2}\bigg)\braket{\vec{p'}_1\vec{p'}_2|iT|\vec{p}_1\vec{p}_2}=\int d^4x_1\int d^4x_2\int d^4x_3\\
				&\cdot \int d^4x_4e^{i(p'_1\cdot x_1+p'_2\cdot x_2-p_1\cdot x_3-p_2\cdot x_4)}\frac{\braket{0|T\{\phi(x_1)\phi(x_2)\phi(x_3)\phi(x_4)e^{-i\frac{\lambda}{4!}\int d^4x\phi^4}\}|0}}{\braket{0|T\{e^{-i\frac{\lambda}{4!}\int d^4x \phi^4}\}|0}}.
			\end{split}
		\end{equation}  
		...
		\begin{equation}
			\begin{split}
				&\frac{i}{p'_1-m^2}\frac{i}{p'_2-m^2}\frac{i}{p_1-m^2}\frac{i}{p_2-m^2}\braket{\vec{p'}_1\vec{p'}_2|iT|\vec{p}_1\vec{p}_2}=\int d^4x_1\int d^4x_2\\
				&\cdot \int d^4x_3\int d^4x_4e^{i(p'_1\cdot x_1+p'_2\cdot x_2-p_1\cdot x_3-p_2\cdot x_4)}\braket{0|T\{\phi(x_1)\phi(x_2)\phi(x_3)\phi(x_4)\}|0}.
			\end{split}
		\end{equation}  
		...
		\begin{equation}
			\begin{split}
				&\frac{i}{p'_1-m^2}\frac{i}{p'_2-m^2}\frac{i}{p_1-m^2}\frac{i}{p_2-m^2}\braket{\vec{p'}_1\vec{p'}_2|iT|\vec{p}_1\vec{p}_2}=\int d^4x_1\int d^4x_2\\
				&\cdot\int d^4x_3\int d^4x_4 e^{i(p'_1\cdot x_1+p'_2\cdot x_2-p_1\cdot x_3-p_2\cdot x_4)}(D_{12}D_{34}+D_{13}D_{24}+D_{14}D_{23}).
			\end{split}
		\end{equation} 
		There are three integrals, of the propagators, to be evaluated. The first
		\begin{equation}
			\begin{split}
				I_1\equiv &\int d^4x_1\int d^4x_2\int d^4x_3\int d^4x_4 \\
				&e^{i(p'_1\cdot x_1+p'_2\cdot x_2-p_1\cdot x_3-p_2\cdot x_4)}D(x_1-x_2)D(x_3-x_4).
			\end{split}
		\end{equation} 
		...
		\begin{equation}
			\begin{split}
				I_2&\equiv \int d^4x_1\int d^4x_2\int d^4x_3\int d^4x_4 \\
				&\qquad e^{i(p'_1\cdot x_1+p'_2\cdot x_2-p_1\cdot x_3-p_2\cdot x_4)}D(x_1-x_3)D(x_2-x_4)\\
		...
		\begin{equation}
			\begin{split}
				&\braket{0|T\{\phi(x_1)\phi(x_2)\phi(x_3)\phi(x_4)\phi^2(x)\}|0}\\
				&=4!D(x_1-x)D(x_2-x)D(x_3-x)D(x_4-x).
			\end{split}
		\end{equation} 
		...
		\begin{equation}
			\begin{split}
				&\frac{i}{p'_1-m^2}\frac{i}{p'_2-m^2}\frac{i}{p_1-m^2}\frac{i}{p_2-m^2}\braket{\vec{p'}_1\vec{p'}_2|iT|\vec{p}_1\vec{p}_2}=\int d^4x_1\int d^4x_2\\
				&\cdot \int d^4x_3\int d^4x_4e^{i(p'_1\cdot x_1+p'_2\cdot x_2-p_1\cdot x_3-p_2\cdot x_4)}(-i\lambda)\\
				&\cdot\int d^4x\braket{0|T\{\phi(x_1)\phi(x_2)\phi(x_3)\phi(x_4)\phi^4(x)\}|0}\frac{1}{\braket{0|T\{1-i\frac{\lambda}{4!}\int d^4x \phi^4\}|0}}.
			\end{split}
		\end{equation} 
		I consider
		\begin{equation}
		...
		\begin{equation}
			\operatorname{VEV}=\braket{0|T\{\phi(x_1)\phi(x_2)\phi(x_3)\phi(x_4)\phi^3(x)\}|0}.
		\end{equation} 
...
\begin{example}
	...
	\begin{equation}
		\begin{split}
			...
			&=\bra{\vec{p'}_2,\vec{p'}_1}T\big\{1-i\int d^4x\mathcal{H}_I(x)\\
			&\qquad+\frac{(-i)^2}{2!}\int d^4x_1\int d^4 x_2\mathcal{H}_I(x_2)\mathcal{H}_I(x_1)+...\big\}\ket{\vec{p}_2, \vec{p}_1}.\\
		\end{split}
		\label{he}
	\end{equation} 
...
\begin{example}
	...
	\begin{center}
		...
		\begin{tikzpicture}
			\begin{feynman}
				% --- Left Side (Final States / Top Left Incoming Photon) ---
				\vertex (i2) at (-3,  2) {$\gamma$};      % Top Left (Incoming photon p_2)
				\vertex (f1) at (-3, -2) {$e^-$};         % Bottom Left (Outgoing electron p'_1)
				
				% --- Interaction Vertices ---
				\vertex (v1) at (-1, 0);                  % Left Vertex
				\vertex (v2) at (1, 0);                   % Right Vertex
				
				% --- Right Side (Initial States / Bottom Right Outgoing Photon) ---
				\vertex (i1) at (3,  2) {$e^-$};          % Top Right (Incoming electron p_1)
				\vertex (f2) at (3, -2) {$\gamma$};       % Bottom Right (Outgoing photon p'_2)
				
				\diagram*{
					% --- Fermion Line flowing from Right to Left ---
					(i1) -- [fermion, edge label'=$p_1$] (v2)
					-- [fermion, edge label'=$p_1-p'_2$] (v1)
					-- [fermion, edge label'=$p'_1$] (f1),
					
					% --- Photons with Momentum Arrows ---
					% Top Left: Photon coming IN to the left vertex
					(i2) -- [photon, momentum=$p_2$] (v1),
					
					% Bottom Right: Photon going OUT from the right vertex
					(v2) -- [photon, momentum=$p'_2$] (f2),
				};
			\end{feynman}
		\end{tikzpicture}
		...
		\begin{tikzpicture}
			\begin{feynman}
				% Bottom row: Incoming particles (Time flows UP)
				\vertex (i1) at (-2, -2) {$e^-$};
				\vertex (i2) at (2, -2) {$\gamma$};
				
				% Middle row: Vertices for the s-channel propagator
				\vertex (v1) at (0, -0.5);
				\vertex (v2) at (0, 1.5);
				
				% Top row: Outgoing particles
				\vertex (f1) at (-2, 3) {$e^-$};
				\vertex (f2) at (2, 3) {$\gamma$};
				
				\diagram*{
					% --- Incoming paths (arrows pointing UP/inward) ---
					(i1) -- [fermion, edge label=$p_1$] (v1),
					(i2) -- [photon, momentum=$p_2$] (v1),
					
					% --- Intermediate virtual state (arrow pointing UP) ---
					(v1) -- [fermion, edge label=$p_1+p_2$] (v2),
					
					% --- Outgoing paths (arrows pointing UP/outward) ---
					(v2) -- [fermion, edge label=$p'_1$] (f1),
					(v2) -- [photon, momentum=$p'_2$] (f2),
				};
			\end{feynman}
		\end{tikzpicture}
...
\begin{example}
	...
	\begin{equation}
		\begin{split}
			...
			&\contraction{,,,,,,,,,,,,,,,,,,,,,,,,}{\vec{p'}}{,..,\vec{k'}|}{\bar{\psi}}
			=(-1)^4N\{[\bar{\psi}(x_2)\slashed A(x_2)\psi(x_2)][\bar{\psi}(x_1)\slashed A(x_1)\psi(x_1)]\}\\
			&\contraction{,,,,,,,,,,,,,,,,,,,}{\vec{p'}}{,,,\vec{k'}|}{\bar{\psi}}
			=N\{[\bar{\psi}(x_1)\slashed A(x_1)\psi(x_1)][\bar{\psi}(x_2)\slashed A(x_2)\psi(x_2)]\}.
		\end{split}
	\end{equation} 	
	...
	\begin{footnotesize}
	 \begin{equation}
		\begin{split}
			...
			&\qquad\qquad \cdot e^{ip'_1 x_1}e^{-ip_1 x_2}\varepsilon_\mu^{r'*}(\vec{p'}_2)\varepsilon_\nu^{r}(\vec{p}_2)e^{ip'_2 x_1}e^{-ip_2 x_2}\\
			&\qquad\qquad+\int d^4x_1\int d^4x_2
			\bar{u}^{s'}(\vec{p'}_1)\gamma^\mu S_{F}(x_1-x_2)\gamma^\nu u^s(\vec{p}_1)\\
			&\cdot e^{ip'_1 x_1}e^{-ip_1x_2}
	...
	\begin{footnotesize}
	 \begin{equation}
		\begin{split}
			...
			&\quad\underbrace{\int d^4x_1
				e^{ip'_2\cdot x_1}e^{ip'_1 x_1}e^{-iq x_1}}_{(2\pi)^4\delta^{(4)}(p'_2+p'_1-q)}\underbrace{\int d^4x_2
				e^{-ip_2 x_2}
