...
\begin{example}
	...
	\begin{equation}
		\ddot{\theta}+\frac{g}{l}\theta\simeq 0\Rightarrow \theta\simeq \theta_0\cos\bigg(\sqrt{\frac{g}{l}t}\bigg).
	\end{equation} 
...
\begin{equation}
	\vec{L}=\vec{x}\times \vec{p}=m\vec{x}\times \vec{v}=m rv\sin(\theta),
	\label{ll1}
\end{equation} 
...
\begin{example}
	...
	\begin{enumerate}
		...
		\begin{equation}
			u_h=A\cos(\theta+\phi_0),
		\end{equation} 
		where $\phi_0$ is some phase. Hereby	
		\begin{equation}
			u=B\cos(\theta+\phi_0)-\frac{Cm_C}{L^2}.
			\label{r10}
		\end{equation} 
		...
		\begin{equation}
			\begin{split}
				\frac{dr}{d\theta}&=\frac{1}{B}\frac{(\theta)}{\cos(\theta)^2}=Br^2\sin(\theta)\\
				&\Rightarrow E_{mek}=\frac{L^2}{2m_Cr^4}\bigg(B^2r^4\sin(\theta)^2+r^2\bigg)+\frac{C}{r}.\\
			\end{split}
			\label{r12}
		\end{equation} 		
...
\begin{equation}
	...
	\end{bmatrix}.
\end{equation}
Note that the first equation is located on the same grid-point as $v_x$, the second on the same grid-point as $v_y$ and the third on the same grid-point as $v_z$. From the derivative of the pressure it is clear that the $v_z$ point must be orthogonal to the pressure point\footnote{$v_z$ is out of the screen on figure \ref{fig:1} since $i_z+1$ minus $i_z$ must land on the pressure point.}. This information is important since the different components of the generalized force is forced to be located on the grid-points corresponding to the rest of the given equation. Writing out the j'th component of the generalized force
\begin{equation}
	\begin{split}
		f_j=& f_{b,j}-\rho\big(v_x\frac{\partial v_j}{\partial x}+v_y\frac{\partial v_j}{\partial y}+v_z\frac{\partial v_j}{\partial z}\big)\\
		&+\eta \big[\frac{\partial^2 v_j}{\partial x^2}+\frac{\partial^2 v_j}{\partial y^2}+\frac{\partial^2 v_j}{\partial z^2}+(\frac{1}{3}+\frac{\zeta}{\eta})\big(\frac{\partial^2 v_x}{\partial j\partial x}+\frac{\partial^2 v_y}{\partial j\partial y}+\frac{\partial^2 v_z}{\partial j\partial z}\big)\big].
	\end{split}
\end{equation}
...
\begin{example}
	...
	\end{equation} 
	\begin{equation}
		x_2=x_1+l\sin(\theta), \quad y_2=-l\cos(\theta),
	\end{equation} 
	...
	\begin{equation}
		\begin{split}
			\dot{x}_2^2+\dot{y}_2^2&=\dot{x}_1^2+2\dot{x}_1l\cos(\theta)\dot{\theta}+l^2\cos(\theta)^2\dot{\theta}^2+l^2\sin(\theta)^2\dot{\theta}^2\\
	...
	\begin{equation}
		\underbrace{\frac{\partial L}{\partial x_1}}_{=0}-\frac{d}{dt}\bigg(\frac{\partial L}{\partial \dot{x}_1}\bigg)=0\Rightarrow \dot{x}_1(m_1+m_2)+m_2l\cos(\theta)\dot{\theta}=const=p_{x_1},
	\end{equation} 
	\begin{equation}
		\begin{split}
			0&=\frac{\partial L}{\partial \theta}-\frac{d}{dt}\bigg(\frac{\partial L}{\partial \dot{\theta}}\bigg)\Rightarrow -m_2l\sin(\theta)(\dot{x}_1\dot{\theta}+g)-m_2l(l\ddot{\theta}+\ddot{x}_1\cos(\theta)-\dot{x}_1\dot{\theta}\sin(\theta))=0.
		\end{split}
	\end{equation} 
	Two of the terms ($\dot{x}_1\dot{\theta}$) cancel, so
	\begin{equation}
		m_2l^2\ddot{\theta}+m_2gl\sin(\theta)+m_2l\ddot{x}_1\cos(\theta)=0.
	\end{equation} 
	...
	\begin{equation}
		\begin{split}
			...
			&\Rightarrow \ddot{x}_1=\frac{m_2l}{m_1+m_2}(\dot{\theta}^2\sin(\theta)-\ddot{\theta}\cos(\theta)).
		\end{split}
	\end{equation} 
	So
	\begin{equation}
		m_2l^2\ddot{\theta}-\frac{m_2^2l^2\cos(\theta)^2}{m_1+m_2}\ddot{\theta}+\frac{m_2^2l^2\cos(\theta)\sin(\theta)}{m_1+m_2}\dot{\theta}^2+m_2lg\sin(\theta)=0.
	\end{equation} 
	...
	\begin{equation}
		m_2\ll m_1 \wedge \theta=\text{small:}\quad \ddot{\theta}+\frac{g}{l}\theta\approx 0 \Rightarrow \theta\approx \theta_0\cos\bigg(\sqrt{\frac{g}{l}}t\bigg).
	\end{equation} 
\end{example}
...
The Hamilton equations can be used to define a quantity called the Poisson bracket; consider the time derivative of a function,
\begin{equation}
	f=f(q_1,q_2,...q_n,p_1,p_2,...p_n,t)
\end{equation}
\begin{equation}
	\begin{split}
		\frac{d f}{dt}
...
\begin{example}
	...
	\end{equation} 
	Evaluate the integral by using $\int \frac{1}{\sqrt{a^2-x^2}}dx=\arcsin(\frac{x}{a})$. Hereby
	\begin{equation}
		t=\sqrt{\frac{m}{k}}\arcsin\bigg(\sqrt{\frac{k}{2E}}q\bigg)-t_0.
	\end{equation} 
